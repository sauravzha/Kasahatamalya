...
source_dir = r"C:\Users\Saurav\Desktop\Kshamatalaya\Photos of leadeship team"
dest_dir = r"C:\Users\Saurav\Desktop\Kshamatalaya\assets\team"

os.makedirs(dest_dir, exist_ok=True)

profiles = [
    ('Mohd Asif Ameen', 'Program Lead', 'Samastipur', ['Asif.jpeg', 'Asif.HEIC']),
    ('Sneha kumari', 'Program manager', 'Samastipur', ['Sneha.HEIC', 'Sneha (1).HEIC']),
    ('Aman Gautam', 'Program Lead', 'Samastipur', ['Aman Gautam .HEIC']),
    ('Deepak Sirsam', 'Program Lead', 'Samastipur', ['Deepak Sirsam.jpg']),
    ('Aman Kumar', 'Program Leader', 'Samastipur', ['Aman Kumar..jpg']),
    ('Sameep Sonkar', 'Program Manager', 'Samastipur', ['Sameep_Sonkar_.jpeg']),
    ('Subham Bhakat', 'State Resource Person', 'Patna, Bihar', ['Subham Bhakat.JPG']),
    ('TINA AGGARWAL', 'Head of Programs, Delhi', 'Delhi', ['Tina.heic', 'tina (1).heic']),
    ('TAMANNA', 'Program Lead', 'Delhi', ['Tamanna.jpg']),
    ('PRIYA', 'Program Leader', 'Delhi', ['Priya.jpg']),
    ('Abhishek Kumar Tiwari', 'Head of Programs, Bihar', 'Bihar', ['Abhishek Tiwari.jpeg', 'Abhishek Tiwari (1).jpeg']),
    ('REENA', 'Program Leader', 'Delhi', ['REENA .webp']),
    ('Ishu', 'Program Manager', 'Bihar', ['Ishu.jpg']),
    ('Tabassum', 'Community Leader', 'Delhi', []), # MaybeIMG-20260115-WA0032.jpg or IMG_8592.HEIC
    ('ANJALI', 'Program Leader', 'Delhi', []),
    ('Pradeep Singh Rathore', 'Program lead ( FFSM )', 'Rajasthan', [])
]

# Find available files in source
available_files = os.listdir(source_dir)

# Unmapped images for fallback
unmapped_files = [f for f in available_files if 'IMG' in f]

cards_html = ''
unmapped_idx = 0

for i, (name, role, loc, possible_files) in enumerate(profiles):
    colors = [
    ...
    for pf in possible_files:
        if pf in available_files:
            matched_file = pf
            break
            
    # If no matched file, try to use one of the unmapped images for the ones we missed
    if not matched_file and unmapped_idx < len(unmapped_files):
        # We will assign IMG files to Tabassum and Anjali or Pradeep arbitrarily since we lack info
        matched_file = unmapped_files[unmapped_idx]
        unmapped_idx += 1
        
    avatar_html = f'<div class="team-avatar-pro">{avatar_letter}</div>'
    
    if matched_file:
        try:
            img_path = os.path.join(source_dir, matched_file)
            ...
            # Resize image to reasonable dimensions for web (e.g. max 600px width/height)
            ...
            avatar_html = f'<div class="team-avatar-pro"><img src="/assets/team/{out_filename}" alt="{title_name}"></div>'
    ...
    <section class="section" id="team" aria-label="Our Leadership Profile" style="background: #FAFCFC; overflow: hidden; position: relative; padding: 8rem 0;">
      ...
      <div class="container" style="position: relative; z-index: 2; max-width: 100%;">
        ...
        <style>
          ...
          /* Added object-position to ensure faces aren't cut */
          .team-avatar-pro img {{
            width: 100%; height: 100%; object-fit: cover; object-position: center 15%; border-radius: 50%;
          }}
...
print("Images processed and HTML updated successfully!")
